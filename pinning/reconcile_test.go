...
func TestNotifyCoalesces(t *testing.T) {
	...
	// Notify far faster than any pass could keep up with. Notify must never
	// block: it is called from the mutation path.
	for slot := uint64(8); slot <= 20; slot++ {
		f.apply(h, slot, f.row(slot, slot))
		rec.Notify("h")
	}

	waitFor(t, "the last root to be pinned", func() bool {
		return hasPinAt(f.pins("h"), pinning.PurposeRoot, h.Root(), true)
	})
	// Whatever it saw on the way, the ledger ends up describing one root: the
	// current one.
	if got := len(f.pins("h")); got != 1 {
		t.Errorf("the ledger holds %d pins for a full head, want exactly 1 (the current root)", got)
	}
}
