...
func TestTrackerRunRecoversArchiveUnavailabilityInProcess(t *testing.T) {
	...
	t.Run("selected generation survives a runtime outage", func(t *testing.T) {
		...
		for {
			got, ok := gatheredMetric(mx, "bloar_unfinalized_retries_total", map[string]string{
				"head": "tip", "reason": metrics.UnfinalizedRetryArchiveUnavailable,
			})
			if ok && got == 1 {
				break
			}
			if time.Now().After(deadline) {
				cancel()
				t.Fatal("tracker did not record the archive-unavailable retry")
			}
...
func gatheredHistogram(mx *metrics.Metrics, family string, labels map[string]string) (uint64, float64, bool) {
	families, err := mx.Registry().Gather()
	if err != nil {
		return 0, 0, false
	}
	for _, candidate := range families {
		...
		for _, sample := range candidate.GetMetric() {
			...
			if !reflect.DeepEqual(gotLabels, labels) || sample.Histogram == nil {
				continue
			}
			return sample.GetHistogram().GetSampleCount(), sample.GetHistogram().GetSampleSum(), true
		}
	}
	return 0, 0, false
}

func TestNewTrackerMaterializesReorgDepthBaseline(t *testing.T) {
	mx := metrics.New()
	_, err := New(Config{
		Headers: fixtureSource(), Archive: &fakeTrackerArchive{}, Head: "unfinalized", HandoffHead: "all",
		WindowSlots: 8, Sources: []BlobSource{{Client: &fakeBlobClient{}}}, Metrics: mx,
	})
	if err != nil {
		t.Fatal(err)
	}

	labels := map[string]string{"head": "unfinalized"}
	count, sum, ok := gatheredHistogram(mx, "bloar_unfinalized_reorg_depth_slots", labels)
	if !ok {
		t.Fatal(`bloar_unfinalized_reorg_depth_slots{head="unfinalized"} is absent before the first reorg`)
	}
	if count != 0 || sum != 0 {
		t.Fatalf("initial reorg histogram = count %d sum %g, want zero", count, sum)
	}

	mx.UnfinalizedReorg("unfinalized", 3)
	count, sum, ok = gatheredHistogram(mx, "bloar_unfinalized_reorg_depth_slots", labels)
	if !ok || count != 1 || sum != 3 {
		t.Fatalf("observed reorg histogram = present %t count %d sum %g, want true/1/3", ok, count, sum)
	}
}
