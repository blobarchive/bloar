...
const (
	...
	// DefaultTransactionAdmissionTimeout bounds how long a publication waits
	// for the edge's single transaction permit. Admission is deliberately a
	// separate budget: queue contention must not consume time reserved for the
	// provider-before-IPNS DHT work after the permit is acquired.
	DefaultTransactionAdmissionTimeout = 20 * time.Second

	// DefaultRequestTimeout bounds the writer's complete local handoff. The
	// extra 30 seconds covers bounded admission plus the AF_UNIX dial, request
	// body, and structured response around a full edge transaction.
...
// ValidateTimeoutBudget enforces the admission, work, request, and server
// ordering shared by the writer and edge. Equality is deliberately refused:
// simultaneous timers recreate the ambiguous "client timed out while the edge
// may have completed" result this protocol exists to avoid.
func ValidateTimeoutBudget(transaction, request, serverWrite time.Duration) error {
	...
	case transactionAdmissionTimeout(transaction) >= request-transaction:
		return fmt.Errorf("edge: admission timeout %s plus transaction timeout %s must be shorter than request timeout %s",
			transactionAdmissionTimeout(transaction), transaction, request)
...
func transactionAdmissionTimeout(transaction time.Duration) time.Duration {
	// Small embedded configurations retain the same ordering without waiting
	// longer for admission than one complete work attempt.
	return min(DefaultTransactionAdmissionTimeout, transaction)
}
