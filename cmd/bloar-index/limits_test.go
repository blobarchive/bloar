...
func TestCheckArchiveLimits(t *testing.T) {
	...
	t.Run("remote limit differing from the durable local expectation is refused", func(t *testing.T) {
		...
		err := checkArchiveLimits(t.Context(), clientFor(t, url), cfgFor(url, 128, 128), true, log)
		if err == nil {
	...
	t.Run("equal to the archive's limit starts", func(t *testing.T) {
		url := archiveStub(t, 64)
		if err := checkArchiveLimits(t.Context(), clientFor(t, url), cfgFor(url, 64, 64), true, log); err != nil {
	...
	t.Run("below the archive's limit starts", func(t *testing.T) {
		url := archiveStub(t, 64)
		if err := checkArchiveLimits(t.Context(), clientFor(t, url), cfgFor(url, 64, 32), true, log); err != nil {
	...
	t.Run("an archive advertising no limit leaves the durable local guard in force", func(t *testing.T) {
		url := archiveStub(t, 0)
		if err := checkArchiveLimits(t.Context(), clientFor(t, url), cfgFor(url, 128, 128), true, log); err != nil {
	...
	t.Run("the unfinalized tracker retains its existing fail-fast scope", func(t *testing.T) {
		...
		err := checkArchiveLimits(t.Context(), clientFor(t, url), cfgFor(url, 64, 64), false, log)
	...
	t.Run("an authoritative 4xx still fails closed", func(t *testing.T) {
		...
		err := checkArchiveLimits(t.Context(), clientFor(t, srv.URL), cfgFor(srv.URL, 64, 64), true, log)
...
func TestArchiveClientWiresAvailabilityToItsConfiguredHead(t *testing.T) {
	url := archiveStub(t, 64)
	...
	cfg.Archive.Head = "base"
	mx := metrics.New()
	client, err := archiveClient(cfg, mx, slog.New(slog.DiscardHandler))
	if err != nil {
	...
	for _, family := range families {
		if family.GetName() != "bloar_index_archive_available" {
			continue
		}
		for _, sample := range family.GetMetric() {
			if len(sample.GetLabel()) == 1 &&
				sample.GetLabel()[0].GetName() == "head" &&
				sample.GetLabel()[0].GetValue() == "base" &&
				sample.GetGauge().GetValue() == 1 {
				return
			}
		}
	}
	t.Fatal(`bloar_index_archive_available{head="base"} = 1 was not exported`)
}
