...
func runUnfinalized(ctx context.Context, cfg *Config, mx *metrics.Metrics, log *slog.Logger) error {
	arch, err := archiveClient(cfg, mx, log)
	if err != nil {
		return err
	}
	if err := checkArchiveLimits(ctx, arch, cfg, true, log); err != nil {
		return err
	}
...
func archiveClient(cfg *Config, mx *metrics.Metrics, log *slog.Logger) (*archclient.Client, error) {
	token, err := cfg.Token()
	if err != nil {
		return nil, err
	}
	return archclient.New(archclient.Config{
		...
		Token:   token,
		Logger:  log,
		ObserveAvailability: func(available bool) {
			mx.IndexArchiveAvailable(cfg.Archive.Head, available)
		},
	})
}
