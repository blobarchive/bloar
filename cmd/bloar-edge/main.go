...
func (c *config) validate() error {
	...
	if c.Control.TransactionTimeout <= 0 ||
		c.Control.TransactionTimeout >= publicationedge.DefaultRequestTimeout {
		return fmt.Errorf("control.transaction_timeout %s must be positive and shorter than the writer default request timeout %s",
			c.Control.TransactionTimeout, publicationedge.DefaultRequestTimeout)
	}
...
func serve(ctx context.Context, cfg *config) (retErr error) {
	...
	router, err := p2p.NewDHT(ctx, host, p2p.MergeBootstrapPeers(explicit, p2p.PublicAminoBootstrapPeers()))
	if err != nil {
		return err
	}
	...
	log.Info("edge started", "peer_id", host.ID(), "ipns_authority", name,
		...
		"metrics", cfg.Metrics.Listen, "restoring", sink.HasState())
