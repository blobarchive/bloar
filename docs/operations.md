...
The availability gauge records the result of the most recently completed
logical archive request after that request's bounded retries. Intermediate
failed attempts and caller cancellation do not change it. Any later successful
request restores `1` in the same process; any non-5xx error response also records
reachability before a protocol or configuration error fails closed.
...
hands both to `bloar-edge` over its private AF_UNIX socket. The handoff has four
strictly ordered defaults:

| budget | config | default |
|---|---|---:|
| edge serialized-transaction admission | fixed liveness bound | `20s` |
...
actual DHT work after admission. Permit contention is capped separately at 20
seconds, leaving 10 seconds in the writer request for the AF_UNIX dial, bounded
request body, and structured response; the server retains another 30 seconds
so its socket deadline cannot race the writer's. The admission and transaction
sum must be strictly below the writer request, and equal outer values are
rejected. The writer transmits the expected transaction timeout and the edge
refuses a mismatch, so independently edited configs fail before any DHT
mutation.
...
Permit admission is bounded independently, and the full transaction deadline
starts only **after** the edge acquires its single publication permit. The
end-to-end transaction timer still includes both intervals. Read
`edge_publication_wait_duration_seconds{operation,outcome}` alongside
`edge_publication_transaction_duration_seconds{operation,outcome}` and
`edge_publication_transactions_total{operation,stage,outcome}` to distinguish a
writer publication from durable-state restore, permit contention from DHT work,
and the terminal stage of a failure. `edge_dht_routing_table_peers` is sampled
at each transaction boundary; its companion timestamp says whether that scalar
is current.

Every failed accepted transaction also produces one structured
`edge publication transaction failed` log. It records the closed operation,
terminal stage and outcome, total and permit-wait time, Provide/PutValue elapsed
time when reached, remaining budget before each DHT stage, the configured
admission and transaction timeouts, submitted identifiers once validation has
reached them, the durable sequence/revision/CID when present, whether the
durable floor changed before failure, and the routing-table size. A failed
writer submission records the authoritative HTTP 422 response; restore has no
HTTP status. The diagnostics add no raw document or record bytes, private keys,
or other secrets. In particular, a `put_record` timeout with
`durable_floor_updated=true` is not a contradiction: the durable floor is
intentionally installed before the network Put and makes retry/restore
monotonic.
...
| `edge_publication_transactions_total{operation,stage,outcome}` | Completed edge transactions split by `publish`/`restore`, terminal stage (`admission`, `validation`, `provide_document`, `persist_state`, `put_record`, or `complete`), and closed outcome. Identifiers and errors are logs, never labels. |
| `edge_publication_transaction_duration_seconds{operation,outcome}` | Whole edge attempt time from before bounded permit admission through completion. Unlike the DHT-stage histogram, this includes permit contention as well as the fresh post-admission work budget. |
| `edge_publication_wait_duration_seconds{operation,outcome}` | Time spent waiting for the serialized publication permit under its separate admission bound. Timeout/cancellation here occurs before, and does not consume, the DHT work budget. |
| `edge_dht_routing_table_peers`, `edge_dht_routing_table_sample_timestamp_seconds` | Amino DHT routing-table size sampled at the latest transaction boundary and the sample time. Peer identities are never labels. |
